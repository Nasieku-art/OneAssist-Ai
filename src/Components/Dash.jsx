...
import { LayoutDashboard,PersonStanding,Speech,Headset,Phone,MessageCircleCheck,Mic,MicOff,Sparkles,Ear,Eye,GraduationCap,Wifi,BookOpen,Users,School,Building2,HeartPulse,Volume2,Loader2,CheckCircle2,ChevronDown,LogOut,Settings,} from "lucide-react";
...
const PROFILES = [
  {
    ...
    desc: {
      Kiswahili: "Sauti husoma kila kitu, hakuna haja ya kuona skrini.",
      English: "Everything is read aloud. Screen-reader friendly.",
    },
    prefs: {
      textToSpeech: true,
      screenReader: true,
  ...
  },
  {
    ...
    desc: {
      Kiswahili: "Maandishi na picha badala ya sauti, arifa za kuona.",
      English: "Text and visuals replace audio, with visual alerts.",
    },
    prefs: {
      textToSpeech: false,
      screenReader: false,
  ...
  },
  {
    ...
    icon: BookOpen,
    name: {
      Kiswahili: "Changamoto za kusoma (Dyslexia)",
      English: "Reading difficulties (Dyslexia)",
    },
    desc: {
      Kiswahili: "Fonti na nafasi rahisi kusoma, pamoja na sauti.",
      English: "Reading-friendly font & spacing, plus read-aloud.",
    },
  ...
  },
  {
    ...
    desc: {
      Kiswahili: "Maandishi makubwa, rangi zinazoonekana wazi, sauti polepole.",
      English: "Large text, high contrast, and slower speech.",
    },
    prefs: {
      largeText: true,
      highContrast: true,
  ...
  },
  {
    ...
    icon: GraduationCap,
    name: {
      Kiswahili: "Kusoma na kuandika kidogo",
      English: "Low literacy / students",
    },
    desc: {
      Kiswahili:
        "Maneno rahisi (AI), sauti, na alama badala ya maandishi marefu.",
      English: "AI-simplified words, audio, and icons over long text.",
    },
  ...
  },
  {
    ...
    icon: Wifi,
    name: {
      Kiswahili: "Maeneo ya vijijini",
      English: "Rural / low connectivity",
    },
    desc: {
      Kiswahili: "Hali ya kutumia data kidogo, bila video au picha nzito.",
      English: "Low-data mode — no heavy images, audio, or video.",
    },
...
const ORG_TYPES = [
  {
    key: "individual",
    label: { Kiswahili: "Mtu binafsi", English: "Individual" },
    icon: PersonStanding,
  },
  {
    key: "school",
    label: { Kiswahili: "Shule", English: "School" },
    icon: School,
  },
  {
    key: "hospital",
    label: { Kiswahili: "Hospitali", English: "Hospital" },
    icon: HeartPulse,
  },
...
function Dash() {
  ...
  const handleVoiceCommand = (transcriptRaw) => {
    ...
    const nav = [
      ...
      {
        keys: ["accessibility", "settings", "mipangilio"],
        tab: "Accessibility Settings",
      },
      {
        keys: ["language", "voice", "lugha", "sauti"],
        tab: "Language & Voice",
      },
      {
        keys: ["simplify", "summarize", "rahisisha", "muhtasari", "ai"],
        tab: "AI Simplify & Summarize",
      },
    ...
    if (!matched) {
      ...
      for (const tgl of toggles) {
        if (tgl.keys.some((k) => transcript.includes(k))) {
          const turnOn =
            transcript.includes("on") || transcript.includes("washa");
          const turnOff =
            transcript.includes("off") || transcript.includes("zima");
          setPreferences((prev) => ({
            ...prev,
            [tgl.key]: turnOn ? true : turnOff ? false : !prev[tgl.key],
          }));
          matched = true;
          feedback = t(
            "Nimebadilisha mpangilio wako.",
            "I updated that setting for you.",
          );
          break;
        }
      }
  ...
  const startVoiceAssistant = () => {
    ...
    if (!recognition) {
      setVoiceFeedback(
        t(
          "Vivinjari hivi haviwezi kutumia sauti.",
  ...
  const startDictation = () => {
    ...
    if (!recognition) {
      setVoiceFeedback(
        t(
          "Uandishi wa sauti hautumiki kwenye kivinjari hiki.",
  ...
  const handleProcessText = async () => {
    ...
    try {
      const langInstruction =
        lang === "Kiswahili" ? "Respond in Kiswahili." : "Respond in English.";
      let prompt;
  ...
  const activeCount = Object.entries(preferences).filter(
    ([k, v]) => v === true,
  ).length;
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div
            className={`text-xl font-bold mb-8 tracking-tight flex items-center gap-2 ${hc ? "text-yellow-300" : "text-teal-600"}`}
          >
            OneAssist{" "}
            <span
              className={`text-xs px-2 py-0.5 rounded-full font-normal ${hc ? "bg-yellow-400 text-black" : "bg-teal-50 text-teal-700"}`}
            >
              AI
            </span>
          </div>
              ...
              <button
                ...
                className={`w-full text-left flex items-center gap-3 px-4 py-2.5 text-sm font-medium rounded-lg transition-colors ${
                  activeTab === tab
                    ...
                    : hc
                      ? "text-yellow-200 hover:bg-gray-900"
      ...
      <div className="flex-1 flex flex-col md:overflow-hidden">
        ...
        >
          <h1 className="text-lg font-semibold">
            {activeTab === "Dashboard"
              ? t("Kituo Chako cha Ufikiaji", "Your Accessibility Hub")
              : activeTab}
          </h1>
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={profileMenuRef}>
              ...
              >
                ...
                <ChevronDown
                  size={16}
                  className={`transition-transform ${profileMenuOpen ? "rotate-180" : ""} ${subtleText}`}
                />
              </button>

              {profileMenuOpen && (
                <div
                  className={`absolute right-0 mt-2 w-44 rounded-xl border shadow-lg py-1.5 z-20 ${hc ? "bg-black border-yellow-400" : "bg-white border-gray-200"}`}
                >
                  <button
                    onClick={() => {
                      setActiveTab("Accessibility Settings");
                      setProfileMenuOpen(false);
                    }}
                    className={`w-full flex items-center gap-2 text-left px-4 py-2 text-sm ${hc ? "text-yellow-200 hover:bg-gray-900" : "text-gray-700 hover:bg-gray-50"}`}
                  >
                    <Settings size={16} />
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-2 text-left px-4 py-2 text-sm font-medium ${hc ? "text-yellow-200 hover:bg-gray-900" : "text-red-600 hover:bg-red-50"}`}
                  >
                    <LogOut size={16} />
                    {t("Toka", "Logout")}
                  </button>
        ...
        <main className="flex-1 overflow-y-auto p-8 space-y-6">
          {activeTab === "Dashboard" && (
            <div className="space-y-6">
              <div
                onClick={() =>
                  say(`Karibu, ${fullName}!`, `Welcome back, ${fullName}!`)
                }
              ...
              >
                <h2 className="text-2xl font-bold">
                  {t("Karibu", "Welcome back")}, {fullName}!{" "}
                </h2>
                <p
                  className={`mt-1 text-sm ${hc ? "text-black/70" : "text-teal-100"}`}
                >
                  {t(
                    "Nafasi yako ya kidijitali imewekwa kulingana na mahitaji yako. (Bofya kusikia)",
                    "Your digital workspace is adapted for your comfort. (Click box to hear audio)",
                  )}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div
                  className={`p-6 rounded-xl border shadow-sm ${cardSurface}`}
                >
                  <p className={`text-sm ${subtleText}`}>
                    {t("Lugha Inayotumika", "Active Language")}
                  </p>
                ...
                >
                  <p className={`text-sm ${subtleText}`}>
                    {t("Mabadiliko Yanayotumika", "Active Adaptations")}
                  </p>
                  <p className="text-xl font-bold mt-1">
                    {activeCount} {t("Yamewashwa", "Enabled")}
                  </p>
                </div>
                <div
                  className={`p-6 rounded-xl border shadow-sm ${cardSurface}`}
                >
                  <p className={`text-sm ${subtleText}`}>
                    {t("Aina ya Mtumiaji", "Account Type")}
                  </p>
              ...
              <div className={`p-6 rounded-xl border shadow-sm ${cardSurface}`}>
                <h3 className="text-base font-semibold mb-1">
                  {t("Unatumia hii kama nani?", "Who are you using this as?")}
                </h3>
                <p className={`text-xs mb-4 ${subtleText}`}>
                  {t(
                    "Hii hutusaidia kubinafsisha uzoefu wako.",
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          setOrgType(o.key);
                          say(
                            `Umechagua ${o.label.Kiswahili}`,
                            `Selected ${o.label.English}`,
                          );
                        }}
                        className={`flex flex-col items-center gap-2 p-4 rounded-xl border text-sm font-medium transition-colors ${
                          active
                            ...
                            : hc
                              ? "border-yellow-400 hover:bg-gray-900"
              ...
              <div
                className={`p-6 rounded-xl border shadow-sm space-y-4 ${cardSurface}`}
              >
                <div>
                  <h3 className="text-base font-semibold">
                    {t("Wasifu wa Ufikiaji", "Accessibility Profiles")}
                  </h3>
                  <p className={`text-xs ${subtleText}`}>
                    {t(
                      "Bofya wasifu mmoja ili kubadilisha mipangilio yote mara moja.",
                      "Tap a profile to instantly adjust every setting to match your needs.",
                    )}
                  </p>
                </div>
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {active && (
                            <CheckCircle2
                              size={16}
                              className={hc ? "text-black" : "text-teal-600"}
                            />
                          )}
                        </div>
                        <p className={`text-xs ${active ? "" : subtleText}`}>
                          {p.desc[lang]}
                        </p>
                      </button>
                    );
          ...
          {activeTab === "Accessibility Settings" && (
            <div className="max-w-4xl space-y-6">
              <div
                className={`p-6 rounded-xl border shadow-sm space-y-6 ${cardSurface}`}
              >
                <div>
                  <h2 className="text-xl font-bold">
                    {t(
                      "Mapendeleo ya Kuona na Kusoma",
                      "Vision & Readability Preferences",
                    )}
                  </h2>
                  <p className={`text-sm ${subtleText}`}>
                    {t(
                      "Rekebisha vipengele vya skrini kulingana na mahitaji yako.",
                      "Fine-tune screen elements to match your exact comfort level.",
                    )}
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  {[
                    {
                      key: "largeText",
                      title: t("Ukubwa wa Maandishi", "Larger Font Scaling"),
                      desc: t(
                        "Huongeza ukubwa wa maandishi kila mahali.",
                        "Scales typography across the whole app for low-vision users.",
                      ),
                    },
                    {
                      key: "highContrast",
                      title: t("Hali ya Rangi Kali", "High Contrast Mode"),
                      desc: t(
                        "Huongeza tofauti ya rangi ili maandishi yaonekane wazi.",
                        "Enhances color differentiation so borders and text pop.",
                      ),
                    },
                    {
                      key: "dyslexiaFont",
                      title: t(
                        "Fonti Rafiki kwa Dyslexia",
                        "Dyslexia-Friendly Font & Spacing",
                      ),
                      desc: t(
                        "Hutumia fonti na nafasi rahisi zaidi kusoma.",
                        "Uses a clearer font with wider letter and line spacing.",
                      ),
                    },
                    {
                      key: "visualAlerts",
                      title: t("Arifa za Kuona", "Visual Alerts"),
                      desc: t(
                        "Badala ya sauti, arifa huonekana kama picha/rangi.",
                        "Replaces audio cues with on-screen flashes, useful for deaf users.",
                      ),
                    },
                    {
                      key: "dataSaver",
                      title: t("Kuhifadhi Data", "Data Saver Mode"),
                      desc: t(
                        "Hupunguza matumizi ya data kwa maeneo yenye mtandao dhaifu.",
                        "Reduces bandwidth use for rural or low-connectivity areas.",
                      ),
                    },
                    {
                      key: "screenReader",
                      title: t("Msomaji wa Skrini", "Screen Reader Support"),
                      desc: t(
                        "Huongeza lebo za ARIA kwa visomaji vya skrini.",
                        "Adds ARIA labelling optimized for screen readers.",
                      ),
                    },
                  ].map((item) => (
                    <div
                      key={item.key}
                      className={`flex items-center justify-between p-4 border rounded-xl ${hc ? "border-yellow-400" : "border-gray-100 bg-gray-50"}`}
                    >
                      <div>
                      ...
                      <button
                        ...
                        className={`px-4 py-1.5 rounded-full text-xs font-semibold shrink-0 ml-4 ${preferences[item.key] ? (hc ? "bg-yellow-400 text-black" : "bg-teal-600 text-white") : hc ? "bg-gray-800 text-yellow-200" : "bg-gray-200 text-gray-600"}`}
                      >
          ...
          {activeTab === "Language & Voice" && (
            <div className="max-w-4xl space-y-6">
              ...
              >
                <div>
                  <h2 className="text-xl font-bold">
                    {t("Lugha na Sauti", "Language & Voice Customization")}
                  </h2>
                  <p className={`text-sm ${subtleText}`}>
                    {t(
                      "Chagua lugha yako pendwayo na mipangilio ya sauti.",
                      "Choose your preferred language and audio speech settings.",
                    )}
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  >
                    <div>
                      <h4 className="text-sm font-semibold">
                        {t("Kusoma kwa Sauti", "AI Text-to-Speech Engine")}
                      </h4>
                      <p className={`text-xs ${subtleText}`}>
                        {t(
                          "Husoma maelekezo ya skrini kwa sauti moja kwa moja.",
                          "Automatically narrates screen instructions out loud.",
                        )}
                      </p>
                    </div>
                    <button
                      onClick={() => togglePreference("textToSpeech")}
                      className={`px-4 py-1.5 rounded-full text-xs font-semibold ${preferences.textToSpeech ? (hc ? "bg-yellow-400 text-black" : "bg-teal-600 text-white") : hc ? "bg-gray-800 text-yellow-200" : "bg-gray-200 text-gray-600"}`}
                    >
                  ...
                  <div
                    className={`p-4 border rounded-xl ${hc ? "border-yellow-400" : "border-gray-100 bg-gray-50"}`}
                  >
                    <h4 className="text-sm font-semibold mb-2">
                      {t("Kasi ya Sauti", "Speech Speed")}:{" "}
                  ...
                  <div
                    className={`p-4 border rounded-xl space-y-2 ${hc ? "border-yellow-400" : "border-gray-100 bg-gray-50"}`}
                  >
                    <h4 className="text-sm font-semibold">
                      {t("Jaribu Sauti", "Test Voice")}
                    </h4>
                    <p className={`text-xs ${subtleText}`}>
                      {t(
                        "Bofya ili kuhakikisha sauti inafanya kazi kwenye kifaa chako.",
                        "Tap to confirm read-aloud is working on your device.",
                      )}
                    </p>
                    <button
                      onClick={() =>
                        say(
                          "Hii ni sauti ya majaribio. Ikiwa unanisikia, sauti inafanya kazi vizuri.",
                          "This is a test of the voice. If you can hear this, read-aloud is working correctly.",
                        )
                      }
                    ...
                    >
                      <Volume2 size={16} />{" "}
                      {t("Sikiliza Sauti ya Majaribio", "Play Test Voice")}
                    </button>
                    {ttsStatus === "speaking" && (
                      <p className="text-xs font-medium text-teal-600">
                        {t("🔊 Inasema sasa...", "🔊 Speaking now...")}
                    ...
                    {ttsStatus === "done" && (
                      <p className={`text-xs font-medium ${subtleText}`}>
                        {t("✓ Imemaliza kusema.", "✓ Finished speaking.")}
                      </p>
                    )}
                    {ttsStatus === "error" && (
                      <p className="text-xs font-medium text-red-500">
              ...
              >
                <div>
                  <h2 className="text-xl font-bold">
                    {t("Msaidizi wa Sauti", "Voice Assistant")}
                  </h2>
                  <p className={`text-sm ${subtleText}`}>
                    {t(
                      "Sema, kwa mfano 'nenda kwenye mipangilio' au 'washa maandishi makubwa'.",
                      "Speak a command, e.g. 'go to settings' or 'turn on large text'.",
                    )}
                  </p>
                </div>
                <button
                ...
                >
                  ...
                  {isListening
                    ? t("Ninasikiliza...", "Listening...")
                    : t("Bofya Uzungumze", "Tap to Speak")}
                </button>
                {voiceTranscript && (
                  <p className="text-sm">
                ...
                {voiceFeedback && (
                  <p
                    className={`text-sm font-medium ${hc ? "text-yellow-300" : "text-teal-700"}`}
                  >
                    {voiceFeedback}
                  </p>
                )}
              ...
              >
                <div>
                  <h2 className="text-xl font-bold">
                    {t(
                      "Andika kwa Sauti (Speech-to-Text)",
                      "Speech-to-Text Dictation",
                    )}
                  </h2>
                  <p className={`text-sm ${subtleText}`}>
                    {t(
                      "Muhimu kwa mtu asiyeweza kuandika au kuona vizuri — zungumza badala ya kuandika.",
                      "Useful if typing or reading is hard — speak instead of typing.",
                    )}
                  </p>
                </div>
                <textarea
                ...
                <div className="flex gap-3">
                  ...
                  >
                    ...
                    {isDictating
                      ? t("Ninasikiliza...", "Listening...")
                      : t("Anza Kuzungumza", "Start Dictating")}
                  </button>
                  <button
                    onClick={() => setDictationText("")}
                    className={`px-4 py-2 rounded-lg text-sm font-medium ${hc ? "bg-gray-800 text-yellow-200" : "bg-gray-200 text-gray-700"}`}
                  >
                    {t("Futa", "Clear")}
                  </button>
                </div>
              ...
              >
                <div>
                  <h2 className="text-xl font-bold flex items-center gap-2">
                    <Sparkles size={20} />{" "}
                    {t(
                      "Kurahisisha na Kufupisha kwa AI",
                      "AI Simplify & Summarize",
                    )}
                  </h2>
                  <p className={`text-sm ${subtleText}`}>
                    {t(
                      "Bandika maandishi magumu — AI itayarahisisha au kuyafupisha.",
                      "Paste any complex text — AI will rewrite it simply, or condense it to the key points.",
                    )}
                  </p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  {aiMode === "simplify" ? (
                    ...
                    >
                      <option value="short">
                        {t("Fupi (sentensi 2-3)", "Short (2-3 sentences)")}
                      </option>
                      <option value="medium">
                        {t("Wastani (aya moja)", "Medium (one paragraph)")}
                      </option>
                    </select>
                  )}
                  <button
                  ...
                  >
                    ...
                    {isSimplifying
                      ? aiMode === "summarize"
                        ? t("Inafupisha...", "Summarizing...")
                        : t("Inarahisisha...", "Simplifying...")
                      : aiMode === "summarize"
                        ? t("Fupisha Maandishi", "Summarize Text")
                        : t("Rahisisha Maandishi", "Simplify Text")}
                  </button>
                </div>
                ...
                {simplifyOutput && (
                  <div
                    className={`p-4 border rounded-xl space-y-2 ${hc ? "border-yellow-400" : "border-teal-100 bg-teal-50"}`}
                  >
                    <div className="flex items-center justify-between">
                      <h4 className="text-sm font-semibold">
                        {aiMode === "summarize"
                          ? t("Muhtasari", "Summary")
                          : t("Matokeo Rahisi", "Simplified Result")}
                      </h4>
                      <button
                        onClick={() =>
                          speakText(
                        ...
                        className={`flex items-center gap-1 text-xs font-semibold ${hc ? "text-yellow-300" : "text-teal-700"}`}
                      >
            ...
            <div className="max-w-4xl space-y-6">
              ...
              >
                <div>
                  <h2 className="text-xl font-bold">
                    {t(
                      "Msaada na Njia za Mawasiliano",
                      "Assistance & Support Channels",
                    )}
                  </h2>
                  <p className={`text-sm ${subtleText}`}>
                    {t(
                      "Tumejitolea kukusaidia kutumia OneAssist AI kwa urahisi.",
                      "We are dedicated to helping you navigate OneAssist AI smoothly.",
                    )}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div
                    className={`p-5 border rounded-xl space-y-3 ${hc ? "border-yellow-400" : "border-gray-100 bg-gray-50"}`}
                  >
                    ...
                    <h3 className="font-semibold">
                      {t(
                        "Msaada wa Papo Hapo wa WhatsApp",
                        "WhatsApp Instant Support",
                      )}
                    </h3>
                    <p className={`text-xs ${subtleText}`}>
                      {t(
                        "Ongea moja kwa moja na msaidizi kupitia WhatsApp, hata kwa data kidogo.",
                        "Chat directly with a human assistant on WhatsApp for low-data guidance.",
                      )}
                    </p>
                    <button
                      onClick={handleWhatsApp}
                      className={`w-full py-2 text-xs font-semibold rounded-lg shadow-sm ${hc ? "bg-yellow-400 text-black" : "bg-teal-800 text-white hover:bg-green-700"}`}
                    >
                      {t("Fungua WhatsApp", "Open WhatsApp Chat")}
                    </button>
                    <p className={`text-[11px] ${subtleText}`}>
                      {t(
                        `Itafungua WhatsApp kwa nambari ${CONTACT_DISPLAY}`,
                        `Opens WhatsApp to ${CONTACT_DISPLAY}`,
                      )}
                    </p>
                  </div>
                  ...
                  >
                    ...
                    <h3 className="font-semibold">
                      {t("Simu ya Kurudishiwa", "Voice Call Back")}
                    </h3>
                    <p className={`text-xs ${subtleText}`}>
                      {t(
                        "Omba timu yetu ikupigie simu ili kukusaidia na mipangilio.",
                        "Request our team to call your phone directly to walk you through settings.",
                      )}
                    </p>
                    <button
                      onClick={handlePhoneCall}
                      className={`w-full py-2 text-xs font-semibold rounded-lg shadow-sm ${hc ? "bg-yellow-400 text-black" : "bg-teal-800 text-white hover:bg-teal-700"}`}
                    >
                      {t("Piga Simu Sasa", "Call Now")}
                    </button>
                    <p className={`text-[11px] ${subtleText}`}>
                      {t(
                        `Itafungua simu kwa nambari ${CONTACT_DISPLAY}`,
                        `Opens your dialer for ${CONTACT_DISPLAY}`,
                      )}
                    </p>
                  </div>
                </div>
              ...
              {orgType !== "individual" && (
                <div
                  className={`p-6 rounded-xl border shadow-sm space-y-3 ${cardSurface}`}
                >
                  <h3 className="font-semibold flex items-center gap-2">
                    <Users size={18} />{" "}
                    {t("Msaada kwa Mashirika", "Support for Organizations")}
                  </h3>
                  <p className={`text-sm ${subtleText}`}>
                    {t(
                  ...
                  <button
                    onClick={() =>
                      say(
                        "Ombi la mafunzo ya kikundi limetumwa",
                        "Group training request sent",
                      )
                    }
                    className={`px-4 py-2 rounded-lg text-xs font-semibold ${hc ? "bg-yellow-400 text-black" : "bg-teal-600 text-white hover:bg-teal-700"}`}
                  >
