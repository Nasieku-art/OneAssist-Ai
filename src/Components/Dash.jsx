...
const speakText = (text, language = 'Kiswahili', rate = 1.0) => {
...
function Dash() {
  ...
  const say = (sw, en) => speakText(t(sw, en), lang, preferences.voiceSpeed);
  ...
  const handleVoiceCommand = (transcriptRaw) => {
    ...
    const nav = [
      ...
      { keys: ['simplify', 'rahisisha', 'ai'], tab: 'AI Simplify' },
    ...
    try {
      ...
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "user",
            content: `Rewrite the following text to be much easier to read. ${levelInstruction} ${langInstruction} Only return the rewritten text, with no preamble or explanation.\n\nText:\n${simplifyInput}`
          }]
        })
      });
      const data = await response.json();
      const text = (data.content || []).map(b => b.text || '').join('\n').trim();
      setSimplifyOutput(text || t('Samahani, imeshindikana kurahisisha.', 'Sorry, something went wrong simplifying that.'));
    } catch (err) {
    ...
    <div className={`flex h-screen font-sans ${surface}`} style={rootStyle}>
      ...
      <aside className={`w-64 hidden md:flex flex-col justify-between p-6 border-r ${hc ? 'bg-black border-yellow-400' : 'bg-white border-gray-200'}`}>
        <div>
          ...
          <nav className="space-y-2">
            {['Dashboard', 'Accessibility Settings', 'Language & Voice', 'AI Simplify', 'Help & Support'].map((tab) => (
              <button
                key={tab}
              ...
              >
                ...
                {tab === 'AI Simplify' && <Sparkles size={18} />}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-8 space-y-6">
          ...
          {activeTab === 'AI Simplify' && (
            <div className="max-w-4xl space-y-6">
              <div className={`p-6 rounded-xl border shadow-sm space-y-6 ${cardSurface}`}>
                <div>
                  <h2 className="text-xl font-bold flex items-center gap-2"><Sparkles size={20} /> {t('Urahisishaji wa Maandishi kwa AI', 'AI Text Simplification')}</h2>
                  <p className={`text-sm ${subtleText}`}>{t('Bandika maandishi magumu — AI itayarahisisha kwa maneno rahisi.', 'Paste any complex text — AI will rewrite it in simpler words.')}</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  >
                    ...
                    {isSimplifying ? t('Inarahisisha...', 'Simplifying...') : t('Rahisisha Maandishi', 'Simplify Text')}
                  </button>
                </div>
                ...
                {simplifyOutput && (
                  <div className={`p-4 border rounded-xl space-y-2 ${hc ? 'border-yellow-400' : 'border-teal-100 bg-teal-50'}`}>
                    <div className="flex items-center justify-between">
                      <h4 className="text-sm font-semibold">{t('Matokeo Rahisi', 'Simplified Result')}</h4>
              ...
              <div className={`p-6 rounded-xl border shadow-sm space-y-6 ${cardSurface}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className={`p-5 border rounded-xl space-y-3 ${hc ? 'border-yellow-400' : 'border-gray-100 bg-gray-50'}`}>
                    ...
                    <button onClick={() => say('Ninafungua mazungumzo ya WhatsApp', 'Opening WhatsApp chat')} className={`w-full py-2 text-xs font-semibold rounded-lg shadow-sm ${hc ? 'bg-yellow-400 text-black' : 'bg-teal-800 text-white hover:bg-green-700'}`}>
                  ...
                  <div className={`p-5 border rounded-xl space-y-3 ${hc ? 'border-yellow-400' : 'border-gray-100 bg-gray-50'}`}>
                    ...
                    <button onClick={() => say('Ombi la simu limepokelewa', 'Callback request received')} className={`w-full py-2 text-xs font-semibold rounded-lg shadow-sm ${hc ? 'bg-yellow-400 text-black' : 'bg-teal-800 text-white hover:bg-teal-700'}`}>
                      {t('Omba Simu', 'Request Phone Callback')}
                    </button>
                  </div>
                </div>
              </div>
